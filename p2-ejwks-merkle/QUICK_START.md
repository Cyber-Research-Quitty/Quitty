...
### 2. Get JWKS Root
```bash
curl http://localhost:8200/jwks/root | jq
```

### 3. Get Key with Proof
```bash
curl http://localhost:8200/jwks/proof/my-key-1 | jq
```
...
curl http://localhost:8200/log/latest | jq
...
curl http://localhost:8200/jwks/proof/my-key-1 | jq '.root'
```
